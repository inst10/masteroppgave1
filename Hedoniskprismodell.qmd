...
model_log <- data_clean |> 
  ...
  lm(
    formula = log(Realpris) ~ BRA + Soverom + Alder + I(Alder^2) + Fellesgjeld + Avstand_km + Boligtype + Eieform + Salgsår + Grunnkrets,
    data = .
    )
...
tbl_regresjon <- data.frame(
  Variabel = rownames(coeftest(model_log2, vcov = vcovHC(model_log2, type = "HC1"))),
  Estimate = coeftest(model_log2, vcov = vcovHC(model_log2, type = "HC1"))[,1],
  Std_Error = coeftest(model_log2, vcov = vcovHC(model_log2, type = "HC1"))[,2],
  t_value = coeftest(model_log2, vcov = vcovHC(model_log2, type = "HC1"))[,3],
  p_value = coeftest(model_log2, vcov = vcovHC(model_log2, type = "HC1"))[,4]
)
...
model_log_leil <- data_leilighet |> 
  ...
  lm(
    formula = log(Realpris) ~ BRA + Soverom + Alder + I(Alder^2) + Fellesgjeld + Avstand_km + Eieform + Salgsår,
    data = .
    )
...
![](images/clipboard-1659538528.png)

![](images/clipboard-2624745057.png)
...
annen måte å predikere priser: OBS dette er bare leiligheter:
...
# Legg til prediksjonene i datasettet
data_leilighet <- data_leilighet %>%
  mutate(Predikert_logpris = prediksjoner$.pred)

# Konverter tilbake fra log-verdi til faktisk pris
data_leilighet <- data_leilighet %>%
...
```{r}
# Predikere kun for selveierleiligheter
data_selveier <- data_leilighet %>% filter(Eieform == "Selveier")
prediksjon_selveier <- predict(model_fit, new_data = data_selveier)

# Predikere kun for borettslagsleiligheter
data_borettslag <- data_leilighet %>% filter(Eieform == "Borettslag")
prediksjon_borettslag <- predict(model_fit, new_data = data_borettslag)
