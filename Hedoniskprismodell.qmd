...
tbl_regresjon %>%
  ...
  colformat_num(j = "p_value", digits = 5) %>%  # Begrenser p-verdien til 5 desimaler
...
### Konfidensintervall for hver periode:
...
regresjonsformel <- Realpris ~ BRA + Soverom + factor(Salgsår) + factor(Eieform) + factor(Grunnkrets) + Alder + I(Alder^2) + Avstand_km
...
```{r}
model_log_avstand <- lm(log(Realpris) ~ BRA + Soverom + Alder + I(Alder^2) + Fellesgjeld + Avstand_km + factor(Boligtype) + factor(Eieform) + factor(Salgsår), data = data_clean)

summary(model_log_avstand)
```
...
Modell kun med leiligheter og uten grunnkrets:

```{r}
model_leil <- lm(log(Realpris) ~ BRA + Soverom + Alder + I(Alder^2) + Fellesgjeld + Avstand_km + factor(Eieform) + factor(Salgsår), data = leiligheter)
